...
import { formtFechaCorta, orderDate, INITIALPERFIL, fechaPerfil } from "../../helpers";
...
const Laboratorio: React.FC = () => {
  ...
  const fecthIntial = (cancelToken: any = null) => {
    getLaboratorios("", user.idpaciente, "", "", "", 1, "", cancelToken)
      .then((rsp: any) => {
  ...
  const utlimoRegistros = (cancelToken: any = null) => {
    getLaboratoriosUltimoRg("", user.idpaciente, user.cedula, cancelToken)
      .then((rsp) => {
        const { data, status } = rsp;
        if (status === 200) {
          if (data) {
            if (data.data.length > 0) {
              ...
              const clone = orderDate(data.data);
              const [laborat] = clone;
              setLaboratorio(laborat);
            }
          }
        }
      })
  ...
  const fecth = (cancelToken: any = null) => {
    ...
    /*
      idafiliado,idpaciente,busqueda,desde,hasta,page,cancelToken
    */
    getLaboratorios(
      "",
      user.idpaciente,
  ...
  const utlimoLaboratorio = () => {
    getLaboratorioUlt(user.idpaciente)
      .then((rsp) => {
        const { data, status } = rsp;
        if (status === 200) {
          if (data.data.length > 0) {
            const [laborat] = data.data;
            setLaboratorio(laborat);
          } else {
            setLaboratorio({});
          }
  ...
  const handleSearch = (e: any) => {
    ...
    formDa.append("op", "Listado-de-imagenologia-interna");
              ...
              <IonRow>
                ...
                <IonCol size="9" className="height-vh-content-right">
                  <div className="pb-2">
                    ...
                    <IonButton
                      className="button-deg fs-12 mr-2"
                      fill="outline"
                    ...
                    <IonButton
                      className="button-deg no-active fs-12"
                      fill="outline"
                      onClick={handelConsultas}
                    >
                      <IonImg
                        src={"./images/estetoscopio.svg"}
                        className="mr-2 filter-white"
                        style={{ width: "20px" }}
                      />
                      Consultas
