...
const Afiliado = () => {
  ...
  const getRegistro = (cancelToken: any = null) => {
    Promise.all([
      getLaboratorios(id, user.idpaciente, "", "", "", page, "", cancelToken),
      getImagenologias(id, user.idpaciente, "", "", "", page, "", cancelToken),
      getConsultas(id, user.idpaciente, "", "", "", page, "", cancelToken),
    ])
      .then((rsp: any) => {
        const [cnst, lab, img] = rsp;
        let dataCnst = cnst.data.data;
        let dataLab = lab.data.data;
        let dataImg = img.data.data;
        const conector = [...dataCnst, ...dataLab, ...dataImg];
        setRegistros((prev: any) => [...prev, ...conector]);
        let ttlCnst = parseInt(cnst.data.totalResults);
        let ttlLab = parseInt(lab.data.totalResults);
        let ttlImg = parseInt(img.data.totalResults);
        let total = ttlCnst + ttlLab + ttlImg;
  ...
  useEffect(() => {
    ...
    getComboAfiliadosId(id, user.id)
      .then((rsp) => {
    ...
    getCentrosProduccion(id, user.id)
      .then((rsp) => {
        const { data, status } = rsp;
        if (status === 200) {
          if (data.data) {
